...
static void set_cpus_allowed_scx(struct task_struct *p, const struct cpumask *new_mask, u32 flags)
...
//static
void reweight_task_scx(struct rq *rq, struct task_struct *p, int newprio)
{
    refresh_scx_weight(p);
}
EXPORT_SYMBOL(reweight_task_scx);
...
static void check_preempt_curr_scx(struct rq *rq, struct task_struct *p, int flags)
{
                                     ...
                                     enum bpf_type_flag *flag)
{
    ...
    if (t != task_struct_type) {
        bpf_log(log, "only access to task_struct is supported\n");
        return -EACCES;
    }
    ...
    bpf_log(log, "no write support to task_struct at off %d\n", off);
    return -EACCES;
}
